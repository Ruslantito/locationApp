 ...
 * predefined time interval or distance. To further save the user's battery, this Service will stop when the
 * user isn't viewing our app which include cases like - the user has locked the device or user is now
 * viewing some other app then ours.</p>
 * <p>This class will get the user location after some time/distance interval and update the
 * database with that location. If the user changes the current city i.e. user move to a different city then
...
public class EasyLocationService extends Service
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    if (intent != null) {
      ...
      runAsBackgroundService = intent.getBooleanExtra(EasyLocationConstants.RUN_AS_BACKGROUND_SERVICE, false);
    }
    getLocationUpdate();
   ...
   * permission, swiching on GPS and getting location. To get location, just call this function.
   */
  ...
  /**
   * Function to check if we have Google Play Service avaialable, without which fusion location won't work
